...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomFpHdle;                    /**< Football_Player handle */
  uint16_t  CustomPagHdle;                  /**< Pulse_Axel_GPS handle */
}CustomContext_t;
 ...
 */
#define COPY_FOOTBALL_PLAYER_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0x40,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_PULSE_AXEL_GPS_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0x41,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for Football_Player +
  ...
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             ...
                             PRIMARY_SERVICE,
                             4,
                             &(CustomContext.CustomFpHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
//    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: FP, error code: 0x%x \n\r", ret);
  }
  else
  {
//    APP_DBG_MSG("  Success: aci_gatt_add_service command: FP \n\r");
  }
  ...
  ret = aci_gatt_add_char(CustomContext.CustomFpHdle,
                          ...
                          &(CustomContext.CustomPagHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
//    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : PAG, error code: 0x%x \n\r", ret);
  }
  else
  {
//    APP_DBG_MSG("  Success: aci_gatt_add_char command   : PAG \n\r");
  }
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_PAG:
      ret = aci_gatt_update_char_value(CustomContext.CustomFpHdle,
                                       CustomContext.CustomPagHdle,
                                       0, /* charValOffset */
                                       SizePag, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
//        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value PAG command, result : 0x%x \n\r", ret);
      }
      else
      {
